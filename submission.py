...
def get_disparity(im1, im2, max_disp, win_size):
    ...
    im1_pad = np.pad(im1, w, mode='constant')
    im2_pad = np.pad(im2, w, mode='constant')
    ...
    for d in range(max_disp+1):
        ...
        shifted = np.roll(im2_pad, -d, axis=1)   # shift left
        # crop back to original padded shape
        shifted = shifted[w:H+w, w:W+w]
        
        # squared difference
        diff2 = (im1 - shifted)**2
        
        # sum over window via convolution
        cost = convolve2d(diff2, kernel, mode='same')
